...
@export_range(1, 16, 1) var far_clipmap_gpu_provider_max_sync_blocks: int = 1
...
func _configure_far_clipmap_node() -> void:
	...
	far_clipmap.gpu_provider_max_sync_blocks = far_clipmap_gpu_provider_max_sync_blocks
...
func _far_clipmap_config_key_for_current_settings() -> String:
	return "%d:%.6f:%.6f:%d:%d:%.6f:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%.6f:%.6f:%d:%d:%.6f:%.6f:%.6f:%.6f:%.6f:%d:%d:%d" % [
		...
		far_clipmap_gpu_provider_max_sync_blocks,
		far_clipmap_surface_normal_strength,
