...
func _sample_height_grid_fast_region(origin_x: float, origin_z: float, step_m: float, count_x: int, count_z: int, world_seed: int, region_size_m: float) -> PackedFloat32Array:
	if use_native_prepared_height_grid:
...
func _sample_height_value_with_corner_entries(x: float, z: float, world_seed: int, corner_entries: Array[Dictionary], corner_weights: Array[float]) -> float:
	var continent: float = TerrainHashScript.fbm(x, z, 52000.0, world_seed + 3, 4)
	...
	var macro: float = (
		continent * 560.0
		+ TerrainHashScript.fbm(x, z, 26000.0, world_seed, 4) * 430.0
		+ TerrainHashScript.fbm(x + 2300.0, z - 1100.0, 12000.0, world_seed + 11, 3) * 140.0
	)
	var ridge: float = ridged_noise(x * 0.8 + z * 0.15, z * 0.65 - x * 0.1, 18000.0, world_seed + 37, 3)
	...
	for corner_index in range(corner_entries.size()):
		...
		for entry_value in entries:
			...
			var sampled: float = _sample_kernel_cached(entry, x, z)
			relief += sampled * weight * runtime_weight * moderation * float(entry["relief_scale_m"]) * (0.58 + upland * 0.38)
			detail += (
				TerrainHashScript.fbm(x, z, 3000.0, int(entry["detail_seed"]), 2)
				* weight
				* runtime_weight
				* moderation
	...
	var valleys: float = valley_mask(x, z, world_seed)
	var valley_cut: float = valleys * (110.0 + upland * 130.0)
	var valley_floor_noise: float = TerrainHashScript.fbm(x, z, 4200.0, world_seed + 401, 2) * 24.0 * valleys
	return f32(macro + relief + detail - valley_cut + valley_floor_noise)


func _sample_kernel_cached(entry: Dictionary, x: float, z: float) -> float:
	...
	for corner in corner_defs:
		...
		for index in range(families.size()):
			...
			if kernel.is_empty():
				continue
			var kernel_id: String = str(kernel.get("id", ""))
			var sampled: float = sample_kernel_field(kernel_id, x, z, world_seed, crx, crz, region_size_m)
			var family_params: Dictionary = pack.families.get(family, {}) as Dictionary
			var runtime_weight: float = float(family_params.get("runtime_weight", 1.0))
			var moderation: float = decisions.kernel_runtime_moderation(float((kernel.get("stats", {}) as Dictionary).get("slope_p95_deg", 0.0)))
			var weight: float = corner_weight * float(biases[index])
			relief += sampled * weight * runtime_weight * moderation * float(family_params.get("relief_scale_m", 0.0)) * (0.58 + upland * 0.38)
			detail += (
				TerrainHashScript.fbm(x, z, 3000.0, world_seed + TerrainHashScript.stable_hash([family]) % 1000, 2)
				* weight
				* runtime_weight
				* moderation
