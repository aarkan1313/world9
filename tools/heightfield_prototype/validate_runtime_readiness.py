...
def _check_gpu_page_review_manifest(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    if int(descriptor_state.get("provider_texture_payload_count", 0)) >= 4:
        check(int(descriptor_state.get("height_image_data_count", -1)) == 0, errors, "gpu_page_review_descriptor_unexpected_height_data")
        check(int(descriptor_state.get("height_texture_rid_count", 0)) >= 4, errors, "gpu_page_review_descriptor_height_texture_rid")
        check(int(descriptor_state.get("rd_owned_rid_descriptor_count", 0)) >= 4, errors, "gpu_page_review_descriptor_rd_owned_rids")
    else:
...
def _check_gpu_page_motion_manifest(manifest: dict[str, Any], errors: list[str]) -> None:
    ...
    for index, sample in enumerate(samples):
        ...
        check(int(descriptor_state.get("height_image_data_count", -1)) == 0, errors, f"gpu_page_motion_descriptor_unexpected_height_data:{index}")
        check(int(descriptor_state.get("height_image_only_count", 0)) >= 4, errors, f"gpu_page_motion_descriptor_height_only:{index}")
        check(int(descriptor_state.get("height_texture_rid_count", 0)) >= 4, errors, f"gpu_page_motion_descriptor_height_texture_rid:{index}")
        check(int(descriptor_state.get("provider_texture_payload_count", 0)) >= 4, errors, f"gpu_page_motion_descriptor_provider_texture:{index}")
        check(int(descriptor_state.get("rd_owned_rid_descriptor_count", 0)) >= 4, errors, f"gpu_page_motion_descriptor_rd_owned_rids:{index}")
