...
quality Godot runtime gate: 14 checks, pass on the current machine after the focused mountain-kernel gallery is included; debug perf raw timings stay in stdout so locked artifacts remain deterministic; runtime readiness checks landform/hydrology report schemas, case counts, field policies, and seam deltas
