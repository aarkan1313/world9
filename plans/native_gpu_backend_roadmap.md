...
70. [x] Add a live landform profile tour scene for visual comparison of current balance, stronger mountains, and compressed scale. It defaults to manual near-terrain review while non-neutral profiles remain outside the native/far prepared-grid path.
...
landform live review: `terrain_landform_profile_tour.tscn` exposes `balanced_current`, `strong_mountains`, and `compressed_scale` on representative sites. It defaults to manual profile switching (`V`) and near-terrain review only; `P` can toggle auto cycling for stationary review, and `N`/`B` still move across selected sites. Far clipmap/profile integration remains a later backend step because non-neutral profiles currently disable the native prepared-grid fast path.
