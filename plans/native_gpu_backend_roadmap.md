...
GPU Godot runtime gate: 7 renderer-enabled checks, pass on the current machine through D3D12 / RTX 5090 Laptop GPU; they prove local RenderingDevice compute, float storage-buffer dispatch/readback, terrain height-page normal encoding into reusable RGBF byte data with CPU parity, main RenderingDevice compute-to-texture storage-image writes, persistent page-normal pipeline reuse, direct Texture2DRD creation from RF/RGBF page bytes, bounded RD page residency, explicit RID cleanup, opt-in far-clipmap consumption of GPU-generated normal bytes plus direct RD page textures through the page texture backend, review-profile wiring through the normal walk-preview scene, and the saved GPU page review scene
