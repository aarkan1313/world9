...
61i. [x] Add review-only `gpu_page_review` quality-profile wiring for the normal walk-preview scene, proving direct RD far-page residency through the same profile system used by live/review scenes while keeping `walk_review` default-safe.
...
GPU page residency: persistent far page height/normal textures now flow through a bounded residency cache with protected active page keys, upload/hit/eviction/MiB diagnostics, a walk-profile page limit, ImageTexture reuse, and an opt-in direct Texture2DRD path. This is still not the default live streaming path; it is the first renderer-owned GPU texture residency step behind explicit gates.
