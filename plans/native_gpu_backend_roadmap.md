...
73. [x] Add an experimental live pass/corridor tour scene, then table it as an acceptance gate because route/corridor facts are not yet native/GPU-page compatible.
74. [ ] Feed corridor/route facts into the native/GPU page path so live corridor review can behave like the landform tour without shrinking the review area or lagging.
75. [ ] Move far page generation/upload toward GPU compute or lower-churn native texture upload after texture-displaced page rendering remains visually accepted.
76. [ ] Replace remaining subtle far/LOD quality shifts through the GPU-resident page/ring path, not more interim mesh/fog/alpha patches. Immediate fixes are still required for black holes, hard seams, crashes, or review-blocking regressions.
...
pass/corridor world facts: `TerrainWorldFacts` emits deterministic route placeholders with `affects_height=false`; the quality gate checks determinism, sample hints, default no-op behavior, and opt-in height shaping when the pass profile knob is enabled. `terrain_pass_corridor_visual_probe_check.gd` adds a review artifact for neutral height, shaped height, cut amount, and corridor-strength mask. `terrain_pass_corridor_tour.tscn` remains an experimental CPU-only spot-check scene, not an acceptance gate, because pass/corridor shaping currently disables native prepared grids until route facts are available to the Rust/native page path. Native-disabled profile changes are bounded: active chunks are queued and only a small near-neighborhood is filled synchronously during startup/review jumps unless an explicit debug/test path opts into full synchronous fallback. Live corridor acceptance returns after native/GPU page generation consumes the same route/corridor fact contract.
