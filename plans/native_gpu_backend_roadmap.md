...
Live visual review is not accepted. A large black terrain slab/rectangle can
still appear near the viewer while runtime diagnostics report full active chunk
residency, no queued work, and no active workers. The latest user capture showed
`chunks 107/107`, `queue 0+0`, `workers 0`, and a far clipmap profile active,
yet a large black slab was visible in front of the camera.

The recent full level-0 underlay, far render-context gating, and streamer
residency halo are useful hardening, but they did not solve this blocker. Current
gates prove many data and renderer contracts; they do not yet prove live visual
validity under fast movement.

Until this is closed, do not promote additional biome, corridor, erosion, local
detail, or near-GPU-page feature work as accepted runtime progress. The next
roadmap step is an instrumented source-identification pass:

- color/tag near chunks and each far clipmap level separately
- report visible mesh provenance, material mode, texture validity, context
  version, and page/chunk key for every terrain surface
- add a renderer capture gate that fails on large near-camera black terrain
  components
- isolate far clipmap, near chunks, GPU provider textures, native workers,
  persistent page mode, and material fallback one switch at a time
...
37. [x] Add a quality Godot runtime gate suite for landform, pass/corridor, hydrology, debug-mode performance, streaming performance, and high-density walk checks.
...
61g. [x] Extract a reusable `TerrainGpuPageNormalBackend` with persistent local RenderingDevice state and cached shader/pipeline RID reuse, then wire it behind an explicit far-clipmap opt-in (`use_gpu_page_normal_backend`) for sync/fallback page commits without changing default native-worker behavior.
61h. [x] Add an opt-in direct `Texture2DRD` far page residency path (`use_gpu_rd_page_textures`) that wraps preencoded RF/RGBF page bytes into main RenderingDevice textures without the ImageTexture upload path, with renderer-enabled tests and explicit RID teardown coverage.
61i. [x] Add review-only `gpu_page_review` quality-profile wiring for the normal walk-preview scene, proving direct RD far-page residency through the same profile system used by live/review scenes before promoting the same far-page path into `walk_review`.
...
61t. [x] Promote `walk_review` far pages to direct `Texture2DRD` residency and main RenderingDevice-computed normal textures, with a renderer-enabled saved-walk-scene gate proving zero ImageTexture uploads.
61u. [x] Add the first isolated GPU macro-height page generation backend and gate. It produces RF height bytes through local RenderingDevice compute and matches the existing GDScript/Rust wide-multiply hash/noise path for positive origins, negative origins, macro relief scale, and regional scale before live integration.
61v. [x] Add the first isolated GPU DEM-kernel sampling primitive and gate. It loads a real normalized runtime kernel, samples it with mirrored bilinear rules on the GPU, and proves parity for negative coordinates, rotation, and UV offsets before full provider blending.
61w. [x] Add the first isolated GPU prepared-provider page assembly gate. It consumes native-style prepared corner entries, flattens real kernel arrays, and computes macro height, region-corner kernel relief, detail noise, and valley shaping in one GPU dispatch with CPU-provider parity; pass/corridor facts remain gated out until they have a GPU-friendly descriptor.
61x. [x] Split GPU prepared-provider inputs into `worldgen9.gpu_provider_page_descriptor.v1` so renderer-device/no-readback integration can reuse the same validated params, entry bytes, and kernel bytes; corridor-shaped descriptors fail fast until route facts are GPU-ready.
61y. [x] Extend bounded GPU page residency to accept externally supplied main-renderer-device `R32F` height texture RIDs, wrap them as `Texture2DRD`, compute normals on the same renderer device, and own/free those RIDs through the existing page lifecycle. This proves the no-readback handoff contract needed by live GPU height-page generation.
61z. [x] Add the first main-renderer-device provider-page texture backend. It consumes the prepared-provider descriptor, dispatches compute directly into an `R32F` height texture RID, keeps dispatch resources alive through page residency cleanup, then hands the texture to the existing `Texture2DRD`/RD-normal path with zero ImageTexture uploads.
61za. [x] Wire the provider-page texture backend into far-clipmap commits behind an opt-in flag. Far pages now split by base region and dispatch one or more prepared-provider GPU blocks into a single renderer-device height texture, then enter bounded RD residency and RD normal compute without CPU image wrappers or ImageTexture uploads.
61zb. [x] Promote the provider-page texture path into the normal walk/GPU review profiles and saved review scenes when a renderer device is available. Gates now prove cumulative provider-page dispatches, direct RD residency, RD normal compute, and zero ImageTexture uploads while headless checks retain fallback behavior.
61zc. [x] Remove the hidden CPU height-page sampling step from provider-texture far-page commits and pace the explicit GPU page review scene to one far level per update. Provider-texture commits now build metadata, dispatch GPU provider blocks directly into renderer-device height textures, and use conservative page bounds for culling while retaining the CPU page sampler as fallback.
61zd. [x] Add a focused GPU page hitch profiler scene and gate. `terrain_gpu_page_profile.tscn` plus `terrain_gpu_page_hitch_profile_check.gd` drive fast renderer-enabled motion, write a per-frame hitch manifest, and expose provider-page phase timings from `TerrainFarClipmapNode.stats()` so recenter hitches can be attributed to provider prep, renderer-device texture dispatch, RD residency/normal compute, mesh setup, or material assignment.
61ze. [x] Bound live synchronous GPU provider-page commits by block count. Multi-region pages that would require several prepared descriptor/dispatch blocks now use the existing async/native height-page path for live motion, while the isolated provider-texture backend gate continues to prove multi-region GPU correctness until staged/prefetched provider descriptors are ready.
61zf. [x] Add an opt-in near terrain page-render path. `TerrainWorldNode.use_gpu_page_chunks` uses `TerrainChunkPageRenderer` to reuse persistent flat chunk meshes by density and displace them from bounded `Texture2DRD` height pages, with async native chunk worker results as the accepted live height producer until the provider descriptor path is staged/prefetched enough for near chunks.
61zg. [x] Promote near page chunks into focused experimental gates, then demote them from saved visual review after live review exposed under-camera rectangular page artifacts. `terrain_gpu_page_profile.tscn`, `terrain_gpu_page_review.tscn`, and the GPU page capture/motion gates now prove direct-RD far pages while requiring zero near GPU page chunks in saved visual review; opt-in near renderer/provider tests continue proving the mechanics separately.
61zh. [x] Split near page chunk rendering mechanics out of `TerrainWorldNode` into `TerrainChunkPageRenderer`, keeping texture residency, shared meshes, page shader/materials, custom AABBs, and flat-normal placeholder data behind a renderer boundary before expanding the near GPU path further.
61zi. [x] Add opt-in staged near provider descriptors. `TerrainWorldNode.use_gpu_provider_page_chunk_descriptor_staging` builds prepared provider blocks into a bounded LRU cache over the update loop, lets direct near provider page commits consume only staged descriptors, and prevents hidden CPU mesh fallback when staging has not caught up by using native-worker height pages or requeueing until staging is ready. `terrain_gpu_provider_chunk_descriptor_staging_check.gd` gates one-chunk, moving 3x3, and full 7x7 review-window backlog direct RD near-provider paths with zero ImageTexture uploads and zero staged-path CPU fallbacks.
61zj. [x] Harden direct provider texture ownership, cache-hit behavior, and timing visibility. Provider-generated RD height textures and dispatch buffers now either transfer into `TerrainGpuPageResidency` or are explicitly released on rejection/failure. Near chunk and far clipmap provider paths also skip renderer-device provider dispatch when the requested page is already resident, using metadata-only page descriptors for cache hits. The experimental staged near-provider hitch profile now records per-frame descriptor stages, provider texture creation time, RD upload deltas, dispatch deltas, and residency state. It no longer reports RID leaks, but still rejects promotion because live review-density provider texture creation remains far over budget.
61zk. [x] Add an optional near GPU page chunk commit-time budget. `TerrainWorldNode.max_gpu_page_chunk_build_ms_per_update` is forwarded through the streaming preview scene and lets tests/review profiles stop applying more near GPU page chunks after an expensive page commit. This caps burst damage from rejected synchronous provider chunks, but it is not the accepted long-term solution because single pages can still exceed the frame budget.
61zl. [x] Harden GPU page profile edge streaming. Streaming previews now keep updating while active chunks are unbuilt or native chunk work is queued/active, `gpu_page_review` uses a larger forward-prefetch buffer with a higher native-worker result budget, and the focused hitch profile records active/built coverage plus an idle-drain phase so edge-load bursts cannot hide behind frame averages.
61zm. [x] Promote full far level-0 underlay for live streaming handoff. The accepted walk/GPU profiles no longer rely on a player-centered clipmap hole, because forward-prefetch and fast recentering can expose that hole behind the viewer as black square terrain. The far level-0 page now remains present under near chunks with a small `-0.08m` bias, while the handoff gate still checks zero height delta at the authoritative near boundary.
61zn. [x] Context-scope far clipmap async payload acceptance. Far native worker requests and returned page payloads now carry a render-context key/version, staged payload commits revalidate that context plus current inner extent, and context changes clear in-flight/staged work before old geometry can attach. The handoff gate now also verifies level-0 full-underlay mesh topology, so metadata cannot hide stale hole-filtered indices.
61zo. [x] Add a streamer residency halo and strict base-window coverage gate. `TerrainStreamer` can now keep an all-direction safety ring resident outside the visible base window while directional prefetch handles travel intent. The GPU page hitch profiler fails on missing base-window chunk keys but allows optional prefetch/halo backlog. This is a residency/data gate only; live review later proved a black slab can still be visible while the base-window metric is green.
61zp. [ ] Add a black-slab visual provenance gate. The gate must identify the source surface (near chunk, far level, fallback material, invalid texture, stale payload, or culling/render state) and fail when a large near-camera black connected component appears while terrain diagnostics report loaded/resident pages.
...
70. [x] Add a live landform profile tour scene for visual comparison of current balance, stronger mountains, and compressed scale. It defaults to manual profile changes, and non-neutral profiles now stay on the native prepared-grid fast path by passing profile scalars through the prepared request.
71. [x] Add deterministic pass/corridor world facts as non-deforming route placeholders before erosion.
72. [x] Add a pass/corridor visual probe artifact for neutral height, shaped height, cut amount, and corridor-strength mask before river/erosion work.
...
76. [ ] Move remaining far page/detail generation toward GPU compute or lower-churn native texture upload after texture-displaced page rendering remains visually accepted; current lower-churn steps are height-page-only native worker payloads, native preencoded RF/RGBF image data, direct-RD height-image-only sync/worker payloads for renderer-computed normals, direct-RD descriptors that skip CPU Image wrappers, a dedicated page texture backend boundary, blend-window page-residency protection, same-shape texture reuse, a renderer-enabled GPU compute proof for terrain normal generation, direct `Texture2DRD` far page residency, main RenderingDevice normal texture compute for far pages, isolated GPU macro-height page generation, isolated GPU DEM-kernel sampling, isolated prepared-provider GPU page assembly, multi-region provider-page texture dispatch, default walk/GPU review profile use of renderer-device provider textures, provider-texture far commits that skip CPU height-sample arrays on the active GPU path, opt-in near page chunks that reuse flat meshes and commit `Texture2DRD` height pages from native worker results through `TerrainChunkPageRenderer`, opt-in staged near provider descriptors with one-chunk, moving 3x3, and full 7x7 review-window backlog direct RD proof, a focused per-frame hitch profile gate for GPU page recenters and active/built chunk coverage, live sync-block capping that routes expensive multi-region far provider pages through async/native fallback, optional near GPU page chunk commit-time pacing, and GPU page profile edge-drain validation. It is not complete until near page chunks pass visual parity/coverage review and the saved-scene hitch profile proves staged near provider chunks preserve live frame pacing; the latest experimental live staged-provider profile is correctness/resource clean but still rejected because descriptor staging is cheap while synchronous provider texture creation costs roughly 125-260ms per near page and about 390ms p95 under budgeted motion. Corridor/hydrology/erosion provider facts plus near/local detail also still need GPU-ready paths without local-device sync/readback.
77. [ ] Replace remaining subtle far/LOD quality shifts through the GPU-resident page/ring path, not more interim mesh/fog/alpha patches. Immediate fixes are still required for black holes, hard seams, crashes, or review-blocking regressions. The current black slab is not subtle LOD debt; it is a P0 visual correctness blocker.
...
far visual clipmap live-scene budgets: streaming/walk previews now keep level 0 as a full underlay beneath authoritative near chunks, using a tiny negative visual bias. This was intended to prevent exposed black holes during fast forward-prefetch/recenter motion, but live review still shows a large black slab; the underlay is not accepted as a complete fix.
...
walk near/far handoff: tested zero height delta at the chunk-window boundary, with visual far level 0 present under the near window. This validates height continuity only; it does not currently eliminate the live black-slab visual failure.
...
fast Godot runtime gate: 20 checks, pass on the current machine, including the quality-profile, visibility-contract, elevation-color material, page texture backend, GPU page residency, walk motion/recenter profile, and forward-prefetch/residency-halo gates. These gates are not sufficient visual acceptance for the active black-slab blocker.
GPU Godot runtime gate: 13 renderer-enabled checks, pass on the current machine through D3D12 / RTX 5090 Laptop GPU; they prove local RenderingDevice compute, float storage-buffer dispatch/readback, terrain height-page normal encoding into reusable RGBF byte data with CPU parity, isolated macro-height page generation into RF bytes with CPU parity for positive origins, negative origins, macro relief scale, and regional scale, isolated DEM-kernel sampling with mirrored bilinear parity for a real runtime kernel, isolated prepared-provider page assembly with macro/kernel/detail/valley CPU-provider parity, main RenderingDevice compute-to-texture storage-image writes, main-renderer-device prepared-provider height texture dispatch, live far-clipmap opt-in use of provider-generated renderer-device height textures across region-local and cross-region pages, persistent page-normal pipeline reuse, direct Texture2DRD creation from RF/RGBF page bytes, externally supplied renderer-device height texture residency, bounded RD page residency, explicit RID cleanup, cache-hit provider dispatch skipping for resident near/far pages, opt-in far-clipmap consumption of GPU-generated normal bytes plus direct RD page textures through the page texture backend, review-profile wiring through the normal walk-preview scene, the saved GPU page review scene, and staged near-provider chunk pages through a full review-window backlog without ImageTexture uploads or hidden CPU fallback
render Godot runtime gate: 7 renderer-backed artifact captures, including the GPU page review capture for direct-RD far page visual acceptance
extended Godot runtime gate: 35 checks, pass on the current machine
...
forward-prefetch residency: the walk profile keeps 49 base chunks plus a movement-biased overlap row and one all-direction residency halo after motion is known, with gates proving the required base window stays filled while optional fringe work may continue
startup prefetch residency: walk setup now preloads the initial camera-forward row plus the residency halo; the motion profiles report base-window misses separately from still-building optional prefetch/halo rows
...
persistent page descriptor reuse: returning to an already GPU-resident far or near page uses a lightweight descriptor and reuses resident height/normal textures instead of rebuilding CPU images or dispatching throwaway provider textures before the cache hit
...
kernel gallery/tour proof: `terrain_kernel_gallery.tscn` builds a compact review grid from real provider-sampled world sites; `terrain_mountain_kernel_gallery.tscn` filters to mountain kernels and shows multiple region variants per kernel; `terrain_kernel_gallery_contact_sheet_check.gd` writes `factory/runtime/godot_kernel_gallery/kernel_gallery_contact_sheet.png` and currently finds all 36 runtime kernel IDs with 0 missing IDs. `terrain_kernel_tour.tscn` uses the normal walk-preview streaming stack from a high camera and advances through diverse selected kernel sites for live review

landform tuning proof: `terrain_landform_profile_compare_check.gd` writes `factory/runtime/godot_landform_profiles/landform_profile_report.json` plus `landform_profile_contact_sheet.png`; it compares `balanced_current`, `strong_mountains`, `medium_scale`, and `compressed_scale` at the same selected sites, verifies the neutral profile does not move default terrain, and gates profile seam deltas. Scale profiles stay review-only until biome/material context exists. Erosion remains later: after base relief/scale, kernel influence, hydrology hints, and first river/pass routing facts are stable
landform live review: `terrain_landform_profile_tour.tscn` exposes `balanced_current`, `strong_mountains`, `medium_scale`, and `compressed_scale` on representative sites. It defaults to manual profile switching (`V`) with far coverage enabled; `P` can toggle auto cycling for stationary review, and `N`/`B` still move across selected sites. This is a proof/review scene, so profile switches and site jumps settle the active near chunks plus all far clipmap levels before returning control instead of trickling visible partial pages. The saved-scene gate now checks `V` and `B` paths for zero pending far work, zero active far workers, and full active chunk residency. Non-neutral profiles stay on the native prepared-grid fast path because prepared requests carry macro/kernel/mountain/scale/valley profile scalars into the Rust backend, and far page requests include the active landform profile in their cache identity.
pass/corridor world facts: `TerrainWorldFacts` emits deterministic route placeholders with `affects_height=false`; the quality gate checks determinism, sample hints, default no-op behavior, and opt-in height shaping when the pass profile knob is enabled. `terrain_pass_corridor_visual_probe_check.gd` adds a review artifact for neutral height, shaped height, cut amount, and corridor-strength mask. The Rust native prepared-grid path now consumes the same deterministic pass/corridor shaping math, so opt-in corridor profiles no longer force chunk/far-page CPU fallback. `terrain_pass_corridor_tour.tscn` remains experimental, not an acceptance gate, until the native/page path is visually stable enough to run it at landform-tour scale without shrinking the review area or lagging.
...
quality Godot runtime gate: includes worldgen capability, kernel gallery/tour, landform profiles, pass/corridor fact/probe checks, hydrology, debug performance, streaming performance, and high-density walk probes; debug perf raw timings stay in stdout so locked artifacts remain deterministic; runtime readiness checks landform/hydrology report schemas, case counts, field policies, and seam deltas
render Godot runtime gate: 7 non-headless capture checks, including the GPU page review visual proof; static preview capture uses native/fast-gray chunk payloads where possible, and locked render artifacts avoid volatile diagnostic overlay text
