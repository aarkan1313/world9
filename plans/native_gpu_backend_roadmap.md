...
61u. [x] Add the first isolated GPU macro-height page generation backend and gate. It produces RF height bytes through local RenderingDevice compute, matches the existing GDScript/Rust wide-multiply hash/noise path for supported positive-origin/default-scale pages, and rejects unsupported negative-origin/profile-scale requests before live integration.
