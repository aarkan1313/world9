...
landform live review: `terrain_landform_profile_tour.tscn` exposes `balanced_current`, `strong_mountains`, and `compressed_scale` on representative sites. It defaults to manual profile switching (`V`) with far coverage enabled; `P` can toggle auto cycling for stationary review, and `N`/`B` still move across selected sites. `V` now queues near chunk refresh and far page refresh through the runtime budget instead of synchronously rebuilding all visible terrain; `terrain_landform_profile_tour_live_key_check.gd` gates the saved scene path. Non-neutral profiles currently disable the native prepared-grid fast path, so profile parity for native/GPU generation remains a later backend step.
