...
61za. [x] Wire the provider-page texture backend into far-clipmap commits behind an opt-in flag. Region-local far pages can now use renderer-device provider compute directly for height textures, then enter bounded RD residency and RD normal compute without CPU image wrappers or ImageTexture uploads; cross-region pages preflight and fall back until a multi-region GPU descriptor exists.
