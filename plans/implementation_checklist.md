...
- [x] Add opt-in `high_density_257_review` quality profile and make the 257v walk perf probe consume its settings and budgets.
...
- [x] Add first isolated GPU DEM-kernel sampling primitive/gate using a real runtime kernel, mirrored bilinear sampling, negative coordinates, rotation, and UV offsets before full provider blending.
...
- [x] Wire the provider-page texture backend into far clipmap commits behind an opt-in flag for region-local pages, with a renderer-enabled gate proving zero sync bytes/images and zero ImageTexture uploads on that live path.
...
- [x] Add opt-in bounded pass/corridor height shaping through the height provider while keeping default terrain unchanged and documenting the native-prepared compatibility policy.
- [x] Fix native-disabled profile refresh in `TerrainWorldNode` so any future non-native-compatible profile rebuilds or requeues active chunks through the CPU fallback path instead of leaving stale native-era meshes.
...
- [x] Harden native-disabled landform profile changes so review scenes queue invalidated chunks and fill only a bounded near-neighborhood on startup instead of forcing unbounded synchronous CPU rebuilds.
- [x] Convert the saved pass/corridor tour into an explicit wider low-density review preset: 33 vertices, radius-2 near window, manual site stepping, `build_when_idle=false`, height-band coloring, native path active while shaping is enabled, and stronger review-only shaping for readability.
- [x] Port deterministic pass/corridor shaping into the Rust native prepared-grid path so corridor profiles no longer force chunk/far-page CPU fallback.
- [x] Keep live pass/corridor streaming acceptance tabled until the native/GPU page renderer is visually stable at full landform-tour scale; keep static visual probe as the current corridor shaping proof.
